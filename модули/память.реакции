...
function УлавливатьВыборСфер(мир)
{
    ...
    this.исполнить = function()
    {
        ...
        window.addEventListener(
            "click",
            function(событие) { self.сообщить(событие); }
        );
    };
    this.сообщить = function(событие)
    {
        var мышь = new THREE.Vector2();
        мышь.x = (событие.clientX / window.innerWidth) * 2 - 1;
        мышь.y = - (событие.clientY / window.innerHeight) * 2 + 1;
        var улавливатель = мир.состояние["улавливатель"];
        var камера = мир.состояние["камера"];
        var корень = мир.состояние["корень"];
        улавливатель.setFromCamera(мышь, камера);
